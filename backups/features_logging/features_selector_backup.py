...
class FeaturesSelector:
    ...
    def get_value_betting_features(
        ...
        if target not in self.value_betting_features:
            NBALogger.log_warning(logger, "Target {target} no tiene características de value betting definidas"))
            return []
        ...
        try:
                ...
                try:
                    ...
                    if isinstance(filtered_correlations, pd.Series):
                        ...
                        if len(duplicated_cols) > 0:
                            NBALogger.log_warning(logger, "Se detectaron columnas duplicadas: {duplicated_cols}"))
                        ...
                        try:
                            ...
                            NBALogger.log_warning(logger, "Error al ordenar correlaciones en DataFrame: {sort_err}"))
                    ...
                    NBALogger.log_error(logger, "Error al ordenar correlaciones: {str(e)}"))
                    # En caso de error, usar las características sin ordenar
                    try:
                        high_corr_features = filtered_correlations.index.tolist()
                    except:
                        NBALogger.log_error(logger, "No se pudo obtener índice de correlaciones, usando características disponibles"))
            ...
            try:
                ...
                NBALogger.log_warning(logger, "Error al analizar nulos: {e}, usando todas las características disponibles"))
                available_features = combined_features
        
        except Exception as e:
            NBALogger.log_warning(logger, "Error en análisis avanzado de características: {e}. Usando características básicas."))
            ...
            try:
                ...
                NBALogger.log_warning(logger, "Error secundario al analizar nulos: {e2}, usando todas las características disponibles"))
        ...
        for line in betting_lines:
            ...
            if target_col not in df.columns:
                try:
                    ...
                    NBALogger.log_warning(logger, "No se pudo crear la columna {target_col}, saltando línea {line}"))
                    continue
    ...
    def identify_high_confidence_betting_lines(
        ...
        if target not in ['PTS', 'TRB', 'AST', '3P']:
            NBALogger.log_warning(logger, "Target {target} no soportado para análisis de líneas de alta confianza"))
            return {}
        ...
        if 'Date' in df.columns and not pd.api.types.is_datetime64_any_dtype(df['Date']):
            try:
                ...
                NBALogger.log_warning(logger, "No se pudo convertir la columna Date a datetime"))
        ...
        for line in sorted(betting_lines):
            ...
            if line_col not in recent_df.columns:
                try:
                    ...
                    NBALogger.log_warning(logger, "No se pudo crear la columna {line_col}, saltando"))
                    continue
        ...
        if not high_confidence_lines:
            NBALogger.log_warning(logger, "No se encontraron líneas de alta confianza para {target} con umbral {min_confidence}"))
    ...
    def get_optimal_betting_strategy(
        ...
        if not high_confidence_lines:
            NBALogger.log_warning(logger, "No se encontraron líneas de alta confianza para {target}. Estrategia no disponible."))
            return strategy
        ...
        if strategy['best_lines']:
            ...
            NBALogger.log_warning(logger, "No se encontraron value bets para {target} con ventaja mínima de {min_edge}"))
    ...
    def analyze_market_inefficiencies(
        ...
        """
        if target not in ['PTS', 'TRB', 'AST', '3P']:
            NBALogger.log_warning(logger, "Target {target} no soportado para análisis de ineficiencias"))
            return {}
        ...
        if not high_confidence_lines:
            NBALogger.log_warning(logger, "No se encontraron líneas de alta confianza para {target}"))
            return {}
        ...
        if bookmakers is None:
            ...
            if odds_columns:
                ...
                NBALogger.log_warning(logger, "No se detectaron columnas con odds de las casas de apuestas"))
                return {}
        ...
        # Filtrar por fecha reciente
            ...
            if 'Date' in df.columns:
                try:
                    ...
                    NBALogger.log_warning(logger, "Error al filtrar por fecha: {e}"))
        ...
        for line, line_info in high_confidence_lines.items():
            ...
            if line_col not in recent_df.columns:
                try:
                    ...
                    NBALogger.log_warning(logger, "No se pudo crear columna {line_col}: {e}"))
                    continue
            ...
            for odds_col in line_odds_cols:
                ...
                try:
                    ...
                    NBALogger.log_warning(logger, "Error al procesar odds para {odds_col}: {e}"))
        ...
        if not valuable_lines:
            NBALogger.log_warning(logger, "No se encontraron líneas con alta precisión y odds favorables para {target}"))
        else:
    ...
    def find_best_odds_arbitrage(
        ...
        """
        if target not in ['PTS', 'TRB', 'AST', '3P']:
            NBALogger.log_warning(logger, "Target {target} no soportado para análisis de arbitraje"))
            return []
        ...
        if not over_odds_cols or not under_odds_cols:
            NBALogger.log_warning(logger, "No se encontraron suficientes columnas de odds para {target}"))
            return []
        ...
        if arbitrage_opportunities:
            ...
            for i, arb in enumerate(arbitrage_opportunities[:3]):
                ...
                NBALogger.log_warning(logger, "No se encontraron oportunidades de arbitraje para {target}"))
    ...
    def compare_line_movements(
        ...
        """
        if target not in ['PTS', 'TRB', 'AST', '3P']:
            NBALogger.log_warning(logger, "Target {target} no soportado para análisis de movimientos de línea"))
            return {}
            
        # Asegurar que tenemos datos de fecha
        if 'Date' not in df.columns:
            NBALogger.log_warning(logger, "No se encontró columna de fecha para análisis de movimientos"))
            return {}
        ...
        try:
            ...
            NBALogger.log_warning(logger, "Error al convertir fechas: {e}"))
            return {}
