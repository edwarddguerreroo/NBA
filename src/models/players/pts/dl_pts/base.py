"""
Clase Base para Modelos de Deep Learning PTS
===========================================

Contiene la clase base abstracta que define la interfaz común
para todos los modelos de Deep Learning especializados en PTS.
"""
...
class BaseDLModel(nn.Module, ABC):
    """
    Clase base abstracta para todos los modelos de Deep Learning PTS.
    ...
    def compute_metrics(self, predictions: torch.Tensor, targets: torch.Tensor) -> Dict[str, float]:
        ...
        # Métricas específicas de PTS
        accuracy_3 = np.mean(np.abs(pred_np - target_np) <= 3.0) * 100
        accuracy_5 = np.mean(np.abs(pred_np - target_np) <= 5.0) * 100
        accuracy_7 = np.mean(np.abs(pred_np - target_np) <= 7.0) * 100
        ...
        return {
            ...
            'rmse': rmse,
            'r2': r2,
            'accuracy_3pts': accuracy_3,
            'accuracy_5pts': accuracy_5,
            'accuracy_7pts': accuracy_7
        }
