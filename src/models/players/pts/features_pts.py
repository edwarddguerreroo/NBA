...
class PointsFeatureEngineer:
    ...
    def __init__(self, lookback_games: int = 10):
        ...
        logger.info("PointsFeatureEngineer inicializado con sistema de cache optimizado y features temporales avanzadas")
    ...
    def generate_all_features(self, df: pd.DataFrame) -> List[str]:
        ...
        if 'PTS' in df.columns:
            ...
            logger.error("PTS no encontrado en el dataset - requerido para features de puntos")
            return []
    ...
    def _create_scoring_performance_features_optimized(self, df: pd.DataFrame) -> None:
        ...
        if 'PTS' not in df.columns:
            logger.warning("Columna PTS no encontrada - features de anotación limitadas")
            return
    ...
    def _create_opponent_defensive_features_optimized(self, df: pd.DataFrame) -> None:
        ...
        if 'Opp' in df.columns:
            ...
            try:
                ...
                logger.warning(f"Error calculando features del oponente: {e}")
    ...
    def _create_biometric_and_position_features_optimized(self, df: pd.DataFrame) -> None:
        ...
        if 'BMI' in df.columns:
            try:
                ...
                logger.warning(f"Error categorizando BMI: {e}")
                df['bmi_category'] = 1.0
                df['athletic_build'] = 1
        else:
            df['bmi_category'] = 1.0
        ...
        try:
            ...
            logger.error(f"Error calculando scorer_archetype_score: {e}")
        ...
        try:
            ...
            logger.error(f"Error calculando clutch_scoring_ability: {e}")
        ...
        try:
            ...
            logger.error(f"Error calculando fatigue_adjusted_scoring: {e}")
        ...
        try:
            ...
            logger.error(f"Error calculando shot_selection_quality: {e}")
    ...
    def _apply_correlation_regularization(self, df: pd.DataFrame, features: List[str]) -> List[str]:
        ...
        try:
            ...
            if critical_lost:
                logger.warning(f"Features críticas perdidas: {critical_lost}")
            else:
            ...
            logger.error(f"Error en filtro de correlación: {str(e)}")
    ...
    def generate_correlation_report(self, df: pd.DataFrame, features: List[str]) -> Dict[str, any]:
        ...
        try:
            ...
            logger.error(f"Error generando reporte de correlaciones: {str(e)}")
            return {'error': str(e)}
    ...
    def get_optimal_feature_set(self, df: pd.DataFrame, max_features: int = 35) -> List[str]:
        ...
        if len(all_features) > max_features:
            ...
            logger.info(f"Conjunto óptimo: {len(all_features)} features seleccionadas")
            return all_features
    ...
    def _compile_essential_features(self, df: pd.DataFrame) -> List[str]:
        ...
        if missing_registered:
            logger.warning(f"Features registradas pero no en DataFrame: {missing_registered}")
        ...
        if missing_features:
            logger.error(f"FEATURES FALTANTES EN DATAFRAME: {missing_features}")
    ...
    def _ensure_feature_in_dataframe(self, df: pd.DataFrame, feature_name: str, 
        ...
        if feature_name not in df.columns:
            logger.warning(f"Feature {feature_name} no encontrada en DataFrame - creando con valor por defecto")
    ...
    def _detect_data_leakage_features(self, df: pd.DataFrame) -> List[str]:
                ...
                if feature != 'PTS' and feature not in confirmed_leakage_features:
                    try:
                        ...
                        if correlation > 0.92:  # Umbral muy estricto
                            ...
                            logger.warning(f"CORRELACIÓN EXTREMA detectada: {feature} (r={correlation:.3f})")
                    except:
                        pass
        ...
        for feature in all_leakage_features:
            if feature in df.columns:
                ...
                logger.error(f"DATA LEAKAGE CONFIRMADO - ELIMINANDO: {feature}")
        ...
        for feature in df.columns:
            if any(pattern in feature.lower() for pattern in suspicious_patterns):
                if feature not in leakage_features:
                    leakage_features.append(feature)
                    logger.warning(f"PATRÓN SOSPECHOSO detectado: {feature}")
    ...
    def _apply_leakage_filter(self, df: pd.DataFrame, features: List[str]) -> List[str]:
        ...
        if leakage_features:
            logger.error(f"🚨 APLICANDO FILTRO REVOLUCIONARIO DE DATA LEAKAGE 🚨")
            logger.error(f"  Features ELIMINADAS por data leakage: {len(leakage_features)}")
            for feature in leakage_features:
                logger.error(f"    ❌ ELIMINADO: {feature}")
    ...
    def _prevent_feature_dominance(self, df: pd.DataFrame, features: List[str]) -> List[str]:
        ...
        if dominant_features:
            logger.warning(f"🔥 FEATURES DOMINANTES detectadas: {dominant_features}")
            ...
            if len(dominant_features) > 1:
                ...
                logger.warning(f"  ⚖️ Manteniendo mejor dominante: {best_dominant} (r={feature_correlations[best_dominant]:.3f})")
                logger.warning(f"  ❌ Eliminando dominantes: {features_to_remove}")
