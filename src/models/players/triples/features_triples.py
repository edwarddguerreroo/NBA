...
class ThreePointsFeatureEngineer:
    ...
    def generate_all_features(self, df: pd.DataFrame) -> List[str]:
        ...
        if missing_cols:
            logger.warning(f"Columnas de triples faltantes: {missing_cols}")
        ...
        if len(created_features) > self.max_features:
            created_features = self._apply_feature_selection(df, created_features)
                ...
                try:
                    ...
                    if all(col in self.teams_df.columns for col in required_cols):
                        ...
                        if 'FGA' in merged_data.columns and 'FTA' in merged_data.columns:
                            ...
                            logger.warning("Columnas FGA/FTA no encontradas después del merge")
                            df['team_pace_factor'] = 1.0
                    else:
                        logger.warning(f"Columnas requeridas no encontradas en teams_df: {required_cols}")
                        df['team_pace_factor'] = 1.0
                except Exception as e:
                    logger.warning(f"Error calculando team_pace_factor: {e}")
                    df['team_pace_factor'] = 1.0
        ...
        if 'Opp' in df.columns and self.teams_df is not None:
            if self._register_feature('opponent_3pt_defense', 'opponent_defense'):
                try:
                    ...
                    logger.warning(f"Error calculando opponent_3pt_defense: {e}")
                    # Fallback simple
