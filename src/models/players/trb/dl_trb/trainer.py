...
from .hybrid import HybridTRBPredictor, MultiScaleHybrid, AdaptiveHybrid
...
class DLTrainer:
    ...
    def prepare_data(self, df: pd.DataFrame, 
                    ...
                    feature_columns: Optional[List[str]] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        """
        # Usar features específicas si se proporcionan
        if feature_columns:
            ...
            logger.info(f"Usando features específicas: {len(numeric_cols)} features")
        else:
            # Filtrar columnas de features (excluir target y metadatos)
            feature_cols = [col for col in df.columns 
                           if col not in ['TRB', 'Player', 'Date', 'Team', 'Opp']]
    ...
    def train(self, df: pd.DataFrame, 
        ...
        X, y = self.prepare_data(df, sequence_length, feature_columns)
