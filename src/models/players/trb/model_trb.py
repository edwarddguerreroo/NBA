...
class StackingTRBModel:
    ...
    def __init__(self, enable_neural_network: bool = True, enable_svr: bool = False, 
        ...
        logger.info(f"Ensemble FINAL: {', '.join(model_names)}")
        
        logger.info("Modelo Stacking TRB inicializado con configuración revolucionaria")
        logger.info(f"Modelos habilitados: NN={enable_neural_network}, SVR={enable_svr}, GPU={enable_gpu}")
    ...
    def train(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        """
        logger.info("Iniciando entrenamiento completo del modelo Stacking TRB...")
        ...
        logger.info("Preparando características especializadas para predicción de rebotes...")
        ...
        logger.info(f"Características seleccionadas: {len(features)}")
        logger.info(f"Top 10 características: {features[:10]}")
        ...
        # PASO 4: Entrenar modelo final (SIMPLIFICADO PARA EVITAR ERRORES)
        logger.info("PASO 4/5: Entrenando modelo final...")
        # Usar solo el stacking principal para evitar complejidad excesiva
    ...
    def _optimize_hyperparameters(self, model, param_space: Dict, X_train: pd.DataFrame, 
        ...
        logger.info(f"{model_name} - Mejor MAE: {study.best_value:.4f}")
    ...
    def _setup_stacking_model(self):
        ...
        """
        logger.info("Configurando stacking ensemble AVANZADO especializado...")
        ...
        logger.info(f"Stacking AVANZADO configurado:")
        logger.info(f"  - Nivel 1: {len(rebounding_specialists)} especialistas en rebotes")
        logger.info(f"  - Nivel 1: {len(context_specialists)} especialistas en contexto")
        logger.info(f"  - Nivel 1: {len(interaction_specialists)} especialistas en interacciones")
        logger.info(f"  - Nivel 2: 3 modelos finales con meta-learner ultra-conservador")
        logger.info(f"  - Total: {len(all_specialists)} + 3 modelos en arquitectura de 2 niveles")
    ...
    def _perform_temporal_cross_validation(self, X_train: pd.DataFrame, y_train: pd.Series):
        ...
        for fold, (train_idx, val_idx) in enumerate(tscv.split(X_train), 1):
            try:
                ...
                logger.info(f"CV Fold {fold}/{n_splits}: MAE={mae:.3f}, R²={r2:.3f}")
        ...
        logger.info(f"CV Promedio: MAE={avg_mae:.3f}±{std_mae:.3f}, R²={avg_r2:.3f}")
...
class XGBoostTRBModel:
    ...
    def __init__(self, enable_neural_network: bool = True, enable_svr: bool = False, 
        ...
        logger.info("Modelo XGBoost TRB inicializado con stacking ensemble BALANCEADO")
