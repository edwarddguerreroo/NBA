"""
Ejecutor del Trainer de Deep Learning para Puntos NBA
=====================================================

Pipeline completo para entrenar modelos de Deep Learning especializados
en predicción de puntos usando arquitecturas avanzadas.
"""
...
from src.models.players.pts.dl_pts.config import DLConfig, TransformerConfig, LSTMConfig, GNNConfig, VAEConfig, EnsembleConfig, HybridConfig
from src.models.players.pts.dl_pts.trainer import DLTrainer
from src.models.players.pts.dl_pts.evaluate import ModelEvaluator
from config.logging_config import configure_model_logging

# Configurar logging
logger = configure_model_logging("pts_dl_pipeline")


class PTSDeepLearningPipeline:
    """
    Pipeline completo para entrenamiento de modelos de Deep Learning PTS.
    ...
    def __init__(self, model_type: str = "transformer", use_gpu: bool = True):
        ...
        logger.info(f"Pipeline DL PTS inicializado: modelo={model_type}, dispositivo={self.device}")
    ...
    def load_and_prepare_data(self) -> pd.DataFrame:
        ...
        logger.info("Generando features especializadas para Deep Learning PTS...")
        ...
        df_clean = df_players.dropna(subset=['PTS'])
    ...
    def create_config(self) -> object:
        ...
        if self.model_type == "transformer":
            config = config_class(
                ...
                sequence_length=15,
                input_features=self.num_features,  # Ajustado al número real de features de PTS
                target_feature="PTS"
            )
    ...
    def train_model(self, df: pd.DataFrame) -> dict:
        ...
        # Entrenar modelo
        train_results = self.trainer.train(df, self.feature_engineer.feature_columns)
    ...
    def save_results(self, output_dir: str = "results/pts_dl_model"):
    ...
    def run_complete_pipeline(self) -> dict:
        ...
        logger.info("=== INICIANDO PIPELINE COMPLETO DE DEEP LEARNING PTS ===")
    ...
    def _log_final_summary(self, results: dict):
        ...
        logger.info(f"Precisión ±3pts: {eval_metrics.get('accuracy_3pts', 'N/A'):.1f}%")
        logger.info(f"Precisión ±5pts: {eval_metrics.get('accuracy_5pts', 'N/A'):.1f}%")
        logger.info(f"Precisión ±7pts: {eval_metrics.get('accuracy_7pts', 'N/A'):.1f}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Pipeline de Deep Learning para PTS NBA')
    ...
    pipeline = PTSDeepLearningPipeline(
    ...
    print(f"Precisión ±5pts: {eval_metrics.get('accuracy_5pts', 'N/A'):.1f}%")
