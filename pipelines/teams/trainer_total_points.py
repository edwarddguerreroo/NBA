...
class TotalPointsTrainer:
    ...
    def load_and_prepare_data(self) -> pd.DataFrame:
        ...
        # Usar datos de equipos para crear target de puntos totales
        teams_data = self.teams_df.copy()
        
        # Crear variable target: suma de puntos propios + puntos del oponente
        if 'PTS' not in teams_data.columns or 'PTS_Opp' not in teams_data.columns:
            raise ValueError("Columnas 'PTS' y 'PTS_Opp' necesarias para puntos totales")
        
        teams_data['total_points'] = teams_data['PTS'] + teams_data['PTS_Opp']
        ...
        total_pts_stats = teams_data['total_points'].describe()
        logger.info(f"Estadísticas Puntos Totales:")
    ...
    def train_model(self) -> Dict:
        ...
        logger.info("Entrenando modelo Total Points con ensemble optimizado...")
        self.training_results = self.model.train(self.df, target_col='total_points')
        training_duration = (datetime.now() - start_time).total_seconds()
        
        logger.info(f"Modelo Total Points completado en {training_duration:.1f} segundos")
        
        # Mostrar resultados del entrenamiento
        logger.info("=" * 50)
        logger.info("RESULTADOS DEL ENTRENAMIENTO TOTAL POINTS")
        logger.info("=" * 50)
        ...
        if 'accuracy_20pts' in self.training_results:
            logger.info(f"Accuracy ±20pts: {self.training_results['accuracy_20pts']:.1f}%")
        
        logger.info("=" * 50)
        
        # Generar predicciones (simuladas para compatibilidad)
        logger.info("Generando predicciones simuladas...")
        # Este modelo requiere team1/team2 específicos, usamos valores simulados
        self.predictions = np.random.normal(220, 15, len(self.df))  # Simulación temporal
        ...
        if hasattr(self.model, 'cutoff_date'):
            ...
            if len(test_data) > 0:
                ...
                logger.info(f"Accuracy ±10pts: {accuracy_10pts:.1f}%, ±20pts: {accuracy_20pts:.1f}%")
                ...
                self.training_results.update({
                    ...
                    'final_accuracy_20pts': accuracy_20pts
                })
...
"""
    ...
    def _plot_target_distribution_compact(self, ax):
        ...
        total_pts_values = self.df['total_points']
    ...
    def _plot_predictions_vs_actual_compact(self, ax):
        ...
        if hasattr(self.model, 'cutoff_date'):
            ...
            if len(test_data) > 0:
                ...
                y_true = self.df['total_points'].values
    ...
    def _plot_total_points_range_analysis_compact(self, ax):
        ...
        if range_names:
            ...
            ax.set_ylabel('MAE')
            ax.set_title('MAE por Rango de Puntos Totales', fontweight='bold')
    ...
    def _plot_over_under_analysis_compact(self, ax):
        ...
        # Simular líneas Over/Under típicas
        over_under_lines = [200, 210, 220, 230, 240]
        ...
        for line in over_under_lines:
            # Calcular accuracy para esta línea
            true_over = (y_true > line).astype(int)
            pred_over = (y_pred > line).astype(int)
            
            accuracy = np.mean(true_over == pred_over) * 100
            
            line_names.append(f'O/U {line}')
            accuracies.append(accuracy)
        
        if line_names:
            bars = ax.bar(line_names, accuracies, color='lightsteelblue', alpha=0.8)
            ...
            ax.set_title('Accuracy Over/Under por Línea', fontweight='bold')
            ax.set_ylim(0, 100)
            ax.grid(axis='y', alpha=0.3)
    ...
    def save_results(self):
        ...
        # Guardar modelo
        model_path = os.path.normpath(os.path.join(self.output_dir, 'total_points_model.pkl'))
        ...
        report = {
            'model_type': 'NBA Total Points Ensemble',
        ...
        if self.predictions is not None:
            predictions_df = self.df[['Team', 'Date', 'Opp', 'PTS', 'PTS_Opp', 'total_points']].copy()
            predictions_df['total_points_predicted'] = self.predictions
            predictions_df['error'] = self.predictions - self.df['total_points']
        ...
        files_summary = {
            'model_file': 'total_points_model.pkl',
    ...
    def run_complete_training(self):
        ...
        """
        logger.info("Iniciando pipeline de entrenamiento Total Points...")
